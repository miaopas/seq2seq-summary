...
class LorenzConstFGenerator(AbstractGenerator):
    ...
    def build_generator(self, config):
        self.config = config
        self.x = np.random.normal(size=(config['n_init'], self.config['K']))
        self.y = np.random.normal(size=(self.config['n_init'],
                                        self.config['K'], self.config['J']))
...
class LorenzRandFGenerator(LorenzConstFGenerator):
    ...
    def generate(self,
        ...
        while batch_idx < data_num - 1:
            ...
            inputs_sub = np.random.uniform(low=self.config['f_low'],
                                           high=self.config['f_high'],
                                           size=(self.config['n_init'],
                                                 path_len - 1,
                                                 self.config['K']))
