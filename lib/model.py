...
class LorentzModel(pl.LightningModule):
    def __init__(self, hid_dim, num_layers):
        super().__init__() 
        input_dim = output_dim = CONFIG.lorentz.K
        self.rnn = nn.LSTM(input_size=input_dim, hidden_size=hid_dim, num_layers=num_layers)
        self.dense = nn.Linear(hid_dim, output_dim)
        self.save_hyperparameters()
...
if __name__ == "__main__":
    ...
    import pickle
    with open('lorentz_1_10_32_large.pkl', 'rb') as f:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath="checkpoints", save_top_k=4, monitor="valid_loss",filename="LorentzRNN-{epoch:02d}-{valid_loss:.2e}")
    trainer = Trainer(accelerator="gpu", devices=1,
                max_epochs=300,
                precision=32,
                logger=TensorBoardLogger("runs", name="lorentz_seq_rnn"),
                callbacks=[checkpoint_callback])
